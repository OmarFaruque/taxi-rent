...
if ( ! class_exists( 'Woo_Wallet_Wallet' ) ) {
    ...
    class Woo_Wallet_Wallet {
        ...
        public function get_wallet_balance( $user_id = '', $context = 'view' ) {
            ...
            return 'view' === $context ? $this->wallet_balance : number_format( $this->wallet_balance, 2, '.', '' );
        }
        ...
        private function recode_transaction( $amount, $type, $details ) {
            ...
            if ( $wpdb->insert( "{$wpdb->base_prefix}woo_wallet_transactions", apply_filters( 'woo_wallet_transactions_args', array( 'blog_id' => $GLOBALS['blog_id'], 'user_id' => $this->user_id, 'type' => $type, 'amount' => $amount, 'balance' => $balance, 'currency' => 'USD', 'details' => $details, 'date' => current_time('mysql') ), array( '%d', '%d', '%s', '%f', '%f', '%s', '%s', '%s' ) ) ) ) {
