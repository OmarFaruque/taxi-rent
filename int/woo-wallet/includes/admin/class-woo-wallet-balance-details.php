    ...
    public function prepare_items() {
        ...
        foreach ( $wp_user_search->get_results() as $user) {
            $data[] = apply_filters('woo_wallet_balance_details_list_table_item_data', array(
                ...
                'balance'  => woo_wallet()->settings_api->get_option( 'currency_symbol', '_wallet_settings_general', '$' ) . number_format((float)woo_wallet()->wallet->get_wallet_balance( $user->ID ), 2, '.', ''),
                'actions'  => ''
            ), $user);
        }
