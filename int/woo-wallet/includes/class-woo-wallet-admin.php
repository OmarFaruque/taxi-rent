...
if ( ! class_exists( 'Woo_Wallet_Admin' ) ) {
    ...
    class Woo_Wallet_Admin {
        ...
        public function admin_menu() {
            $woo_wallet_menu_page_hook = add_menu_page( 'TeraWallet', 'TeraWallet', 'manage_options', 'woo-wallet', array( $this, 'wallet_page' ), '', 59);
            add_action( "load-$woo_wallet_menu_page_hook", array( $this, 'add_woo_wallet_details' ) );
            $woo_wallet_menu_page_hook_add = add_submenu_page( '', __( 'Woo Wallet', 'woo-wallet' ), __( 'Woo Wallet', 'woo-wallet' ), 'manage_options', 'woo-wallet-add', array( $this, 'add_balance_to_user_wallet' ) );
            add_action( "load-$woo_wallet_menu_page_hook_add", array( $this, 'add_woo_wallet_add_balance_option' ) );
            $woo_wallet_menu_page_hook_view = add_submenu_page( '', __( 'Woo Wallet', 'woo-wallet' ), __( 'Woo Wallet', 'woo-wallet' ), 'manage_options', 'woo-wallet-transactions', array( $this, 'transaction_details_page' ) );
        ...
        public function admin_scripts() {
            ...
            // register styles
            wp_register_style( 'woo_wallet_admin_styles', woo_wallet()->plugin_url() . '/inc/woo-wallet/assets/css/admin.css', array(), WOO_WALLET_PLUGIN_VERSION);

            // Register scripts
            wp_register_script( 'woo_wallet_admin_product', woo_wallet()->plugin_url() . '/inc/woo-wallet/assets/js/admin/admin-product' . $suffix . '.js', array( 'jquery' ), WOO_WALLET_PLUGIN_VERSION);
            wp_register_script( 'woo_wallet_admin_order', woo_wallet()->plugin_url() . '/inc/woo-wallet/assets/js/admin/admin-order' . $suffix . '.js', array( 'jquery', 'wc-admin-order-meta-boxes' ), WOO_WALLET_PLUGIN_VERSION);
        ...
        public function add_balance_to_user_wallet() {
            ...
            <div class="wrap">
                ...
                <form id="posts-filter" method="post">
                    <table class="form-table om2">
                        <tbody>
                            <tr>
                                <th scope="row"><label for="balance_amount"><?php echo __( 'Amount', 'woo-wallet' ) . ' ( ' .  $currency . ' )'; ?></label></th>
                                <td>
            ...
            if ( $_SERVER['REQUEST_METHOD'] === 'POST' && isset( $_POST['woo-wallet-admin-adjust-balance'] ) && wp_verify_nonce( $_POST['woo-wallet-admin-adjust-balance'], 'woo-wallet-admin-adjust-balance' ) ) {
                ...
                if ( $user_id != NULL && ! empty( $user_id ) && $amount != NULL && ! empty( $amount ) ) {
                    ...
                    if ( 'credit' === $payment_type) {
                        // echo 'adjast balance user id credit: ' . $user_id . '<br/>';
        ...
        public function add_woocommerce_account_endpoint_settings( $settings) {
            $settings_fields = apply_filters( 'woo_wallet_endpoint_settings_fields', array(
                array(
                    'title' => __( 'My t Wallet', 'woo-wallet' ),
        ...
        public function woocommerce_custom_nav_menu_items( $endpoints) {
            $endpoints[get_option( 'woocommerce_woo_wallet_endpoint', 'woo-wallet' )] = __( 'My Wallet 2', 'woo-wallet' );
            return $endpoints;
        }
