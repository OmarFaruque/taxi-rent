                ...
                <form method="post" action="">
                    <div class="woo-wallet-add-amount">
                        ...
                        <div id="paymentType" class="form-group">
                            ...
                            <?php if(woo_wallet()->settings_api->get_option('paypal', '_wallet_settings_general') == 'on' && class_exists('WC_Gateway_Paypal')): ?>
                                <label for="paypal">
                                    <input type="radio" name="payment_type" id="paypal" value="paypal"> <?php _e('Paypal', 'wp_trawallet'); ?>
                                </label>
                            <?php endif; ?>
                            <?php if(woo_wallet()->settings_api->get_option('payu', '_wallet_settings_general') == 'on' && class_exists( 'WC_Gateway_PayU' ) ): ?>
                                <label for="payu">
                                    <input type="radio" name="payment_type" id="payu" value="payu"> <?php _e('PayU', 'wp_trawallet'); ?>
                                </label>
                            <?php endif; ?>
                            <?php if(woo_wallet()->settings_api->get_option('stripe', '_wallet_settings_general') == 'on' && class_exists('WC_Gateway_Stripe')): ?>
                                <label for="stripe">
                                    <input type="radio" name="payment_type" id="stripe" value="stripe"> <?php _e('Stripe', 'wp_trawallet'); ?>
                                </label>
                            <?php endif; ?>
                            <?php if(woo_wallet()->settings_api->get_option('coinbase_commerce', '_wallet_settings_general') == 'on' && class_exists('WC_Gateway_Coinbase') ): ?>
                                <label for="commerce">
                                    <input type="radio" name="payment_type" id="commerce" value="commerce"> <?php _e('Coinbase Commerce', 'wp_trawallet'); ?>
                                </label>
                            <?php endif; ?>
                        </div>
                        <!-- <input type="submit" name="woo_add_to_wallet" class="woo-add-to-wallet" value="<?php // _e( 'Add', 'woo-wallet' ); ?>" /> -->
                        <button class="btn btn-primary mt-1 ml-auto" id="continue_topup"><?php _e('Continue', 'trawallet'); ?></button>
                    </div>
                </form>
                </div>
            ...
            <?php } else if ( apply_filters( 'woo_wallet_is_enable_transfer', 'on' === woo_wallet()->settings_api->get_option( 'is_enable_wallet_transfer', '_wallet_settings_general', 'on' ) ) && ( ( isset( $_REQUEST['wl'] ) && 'transfer' === $_REQUEST['wl'] ) ) ) { ?> 
               ...
               <form method="post" action="">
                    <p class="woo-wallet-field-container form-row form-row-wide">
                        ...
                        <input type="number" placeholder="<?php _e('User SK', 'trawallet'); ?>" name="user_sk" id="user_sk" class="user_sk form-control w-100">
                    </p>
                    <p class="woo-wallet-field-container form-row form-row-wide">
